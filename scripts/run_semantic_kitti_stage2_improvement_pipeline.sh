...
FUSED_MIN_VISIBLE_POINTS="${FUSED_MIN_VISIBLE_POINTS:-512}"
TRAIN_SAMPLE_RETRIES="${TRAIN_SAMPLE_RETRIES:-12}"
FUSED_TRAIN_FALLBACK="${FUSED_TRAIN_FALLBACK:-lidar-only}"
...
COMMON_ARGS=(
  ...
  --fused-min-visible-points "$FUSED_MIN_VISIBLE_POINTS"
  --train-sample-retries "$TRAIN_SAMPLE_RETRIES"
  --fused-train-fallback "$FUSED_TRAIN_FALLBACK"
)
