...
1. `extra_feature_mode=zeros`: tests whether the gain comes from the extra-point
   training distribution rather than learned image content.
2. `ipfp_detach`: tests whether gradients through IPFP are helping or damaging
   the backbone.
3. LiDAR-only continued fine-tune for `30000` extra steps: controls for extra
   optimization time after the 60000-step LiDAR baseline.
